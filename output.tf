...
output "tg_names" {
  value = slice(
    concat(aws_lb_target_group.tg_no_log.*.name, [""]),
    0,
    var.number_target_group_create,
  )
}

output "tg_names_arn" {
  value = zipmap(
    aws_lb_target_group.tg_no_log.*.name,
    aws_lb_target_group.tg_no_log.*.arn,
  )
}

output "tg_arn" {
  value = slice(
    concat(aws_lb_target_group.tg_no_log.*.arn, [""]),
    0,
    var.number_target_group_create,
  )
}
...
output "tg_arn_suffix" {
  value = slice(
    concat(aws_lb_target_group.tg_no_log.*.arn_suffix, [""]),
    0,
    var.number_target_group_create,
  )
}
